...
1. Resolve the skill directory to an **absolute path** (the directory holding `scripts/cli.js`), referred to below as `$SKILL_DIR`.
2. Always invoke by absolute path — `node "$SKILL_DIR/scripts/cli.js" <command> [address]`. Do NOT rely on a relative `cd` into the skill directory: some agent shell tools mangle a leading-dot path segment (a `.hermes/skills/...` path has been observed becoming `cd hermes`, failing with exit 126 before the command runs).
3. Run the appropriate command:

   ```bash
   SKILL_DIR=/absolute/path/to/pharos-rwa-manager   # the directory holding scripts/

   node "$SKILL_DIR/scripts/cli.js" vaults                  # market overview
...
6. Allow at least 30 seconds before treating a command as hung: when the R25 API is unreachable, R25-backed vaults wait out an 8-second deadline and then degrade to on-chain reads (the reason lands in `errors[]` as an `r25` entry).
