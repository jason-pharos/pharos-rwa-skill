...
// src/util/http.ts
var PHAROS_HEADERS = { origin: "https://port.pharos.xyz", referer: "https://port.pharos.xyz/" };
...
function cacheDir() {
  const base = process.env.PHAROS_RWA_CACHE_DIR || join(homedir() || tmpdir(), ".cache", "pharos-rwa");
  try {
...
function writeCache(name, value) {
  const w = { savedAt: Math.floor(Date.now() / 1e3), value };
  try {
    writeFileSync(join(cacheDir(), `${name}.json`), JSON.stringify(w));
  } catch {
  }
}
function readCache(name, ttlSec) {
  try {
    const raw = readFileSync(join(cacheDir(), `${name}.json`), "utf8");
    const w = JSON.parse(raw);
    if (Math.floor(Date.now() / 1e3) - w.savedAt > ttlSec) return null;
    return w.value;
  } catch {
    return null;
  }
...
function configUrl() {
  return process.env.PHAROS_RWA_CONFIG_URL || `https://raw.githubusercontent.com/${OWNER}/${REPO}/main/config/vaults.json`;
}
async function loadRegistry(opts) {
  if (opts.noRemote || process.env.PHAROS_RWA_NO_REMOTE) return DEFAULT_REGISTRY;
  const cached = readCache(CACHE_KEY, TTL_SEC);
...
// src/sources/harbor.ts
function parseApy(raw) {
...
async function fetchHarbor() {
  const resp = await fetchJson(`${apiBase()}/omni_port/harbor/summary`);
...
function toNumber2(raw, decimals) {
  return Number(formatUnits(raw, decimals));
}

// src/sources/chain.ts
var DEFAULT_RPC = "https://rpc.pharos.xyz";
var DEFAULT_CHAIN_ID = 1672;
var ERC20_ABI = [
...
async function getErc20Decimals(token, provider) {
  const c = new Contract(token, ERC20_ABI, provider);
  return Number(await c.decimals());
}
async function getErc20Balance(token, holder, provider) {
  const c = new Contract(token, ERC20_ABI, provider);
...
function dayDiff(fromSec, toSec) {
  return Math.round((toSec - fromSec) / 86400);
}
function windowState(startSec, endSec, now) {
...
// src/logic/actionPeriod.ts
function build(startTs, endTs, withdrawableTs, source, now) {
...
function messageFor(vault, u, ap) {
  switch (u) {
    ...
    case "open":
      return `${vault}: withdraw window is open now.`;
    case "opening-soon":
...
// src/update/checkVersion.ts
var CACHE_KEY2 = "version-check";
var TTL_SEC2 = 24 * 3600;
function parts(v) {
  return v.replace(/^v/, "").split(".").map((n2) => Number(n2) || 0);
}
function isNewer(latest, current) {
  const a = parts(latest), b2 = parts(current);
  for (let i = 0; i < Math.max(a.length, b2.length); i++) {
    const x = a[i] ?? 0, y = b2[i] ?? 0;
    if (x > y) return true;
    if (x < y) return false;
  }
  return false;
...
async function checkForUpdate(opts) {
  if (opts.noRemote || process.env.PHAROS_RWA_NO_REMOTE) return void 0;
  let tag = null;
  const cached = readCache(CACHE_KEY2, TTL_SEC2);
  if (cached) tag = cached.tag;
  else {
    try {
      const resp = await fetchJson(`https://api.github.com/repos/${OWNER}/${REPO}/releases/latest`);
      tag = resp.tag_name ?? null;
      if (tag) writeCache(CACHE_KEY2, { tag });
    } catch {
      return void 0;
    }
  }
  if (!tag) return void 0;
  return isNewer(tag, VERSION) ? { current: VERSION, latest: tag } : void 0;
}
...
// src/index.ts
function makeEnvelope(data, errors, updateAvailable) {
  const env = { ok: errors.length === 0, generatedAt: (/* @__PURE__ */ new Date()).toISOString(), data, errors };
  if (updateAvailable) env.updateAvailable = updateAvailable;
  return env;
}
async function safeUpdate(opts) {
  try {
    return await checkForUpdate({ noRemote: opts.noRemote });
  } catch {
    return void 0;
  }
}
...
async function buildPositions(address, opts, errors) {
  ...
  return positions.sort((a, b2) => a.vault.localeCompare(b2.vault));
}
async function runPosition(address, opts) {
  const errors = [];
...
async function runReminders(address, opts) {
  const errors = [];
  const positions = await buildPositions(address, opts, errors);
  return makeEnvelope({ address, reminders: buildReminders(positions) }, errors, await safeUpdate(opts));
}
async function runAdvise(address, opts) {
  const errors = [];
  let market = [];
  try {
    market = await fetchHarbor();
  } catch (e) {
    errors.push({ scope: "harbor", error: String(e.message ?? e) });
  }
  const positions = await buildPositions(address, opts, errors);
...
program2.command("vaults").description("market overview of all harbor vaults").action(async () => {
  const g = globals();
  emit2(await runVaults(runOpts(g)), !!g.pretty);
});
program2.command("position <address>").description("position overview for APC3M/pALPHA").action(async (address) => {
  const g = globals();
  assertAddress(address);
  emit2(await runPosition(address, runOpts(g)), !!g.pretty);
});
program2.command("reminders <address>").description("action-period reminders").action(async (address) => {
  const g = globals();
  assertAddress(address);
  emit2(await runReminders(address, runOpts(g)), !!g.pretty);
});
program2.command("advise <address>").description("market + position + gap advice bundle").action(async (address) => {
  const g = globals();
  assertAddress(address);
  emit2(await runAdvise(address, runOpts(g)), !!g.pretty);
});
program2.command("upgrade").description("self-update cli.js from the latest GitHub Release").action(async () => {
  const g = globals();
  emit2(await runUpgrade(), !!g.pretty);
});
program2.parseAsync().catch((err) => fail(String(err?.message ?? err)));
