...
- `actionPeriod.source` is `config`, `onchain-redeemable`, `r25-api`, or `unavailable`. `r25-api` means the dates came dynamically from the R25 `/dapp/vault/period` endpoint (always current, replaces hardcoded config). If `unavailable`, say the withdraw window is currently unknown.
- **Redeemability-based vaults (VRPC-SemiYearly, VRPC-Weekly), `actionPeriod.source: "onchain-redeemable"`** — these vaults have NO fixed withdraw window; the lock runs from each user's OWN deposit (VRPC-SemiYearly 184 days, VRPC-Weekly 7 days — see `actionPeriod.redeemable.lockDays`) and the contract exposes no per-user deposit or maturity date, so we report live on-chain redeemability instead of dates. VRPC-SemiYearly is ERC-7540 async (redeem must be requested, then settled, then claimed → `pendingRedeemShares`/`claimableRedeemShares` are meaningful); VRPC-Weekly is plain ERC-4626 sync (redeem directly → pending/claimable stay 0, `redeemable.async: false`). Two limitations to convey honestly to the user:
  1. **Dates from R25 tranches when available, otherwise no exact dates.** When `r25.tranches` is present (VRPC-SemiYearly), each tranche has an exact `expirationDate` — the date that deposit's lock expires. Present these as the per-tranche unlock dates. `actionPeriod.redeemable` still tells you how much is actionable RIGHT NOW (`maxRedeemShares`/`maxRedeemValue` etc.). When tranches are absent (VRPC-Weekly or API down), there is NO unlock/maturity date available — do NOT state or guess one; convey the rule as "`lockDays`-day lock from deposit". `depositedDurationDays`, `lockEnd`, and `expectedTotalYield` remain `null` for these vaults.
