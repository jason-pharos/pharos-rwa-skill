...
test('coerces numeric strings to numbers', () => {
  const stringSample = {
    ...
    vaultInfo: { receiptTokenPrice: '1.034775439' },
  };
  const vi = extractVaultInfo(stringSample);
  assert.equal(vi.nav, 1.034775439);
  assert.equal(vi.apy, 0.14);
  assert.equal(vi.minWithdrawalShares, 0.1);
  assert.equal(vi.phases[0].apy, 0.16);
});
